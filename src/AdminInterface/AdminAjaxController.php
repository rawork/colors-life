...
use AdminInterface\AdminController;
use Admin\Admin;
...
class AdminAjaxController extends AbstractController {
	...
	function getComponentList($state, $moduleName = '') 
	{
		...
		if ($this->get('util')->_sessionVar('user')) {
			$modulesAll = $this->get('container')->getModules();
	...
	function getTableList($state, $moduleName) {
		if ($this->get('util')->_sessionVar('user')) {
			$this->get('router')->setParam('state', $state);
			$this->get('router')->setParam('module', $moduleName);
			$uai = new AdminController(new Admin($moduleName), '', array());
...
<div class="control-group" id="copyInput">
  ...
  <div class="controls">
	...
	function makeArchive() {
		...
		$archive = new \Common\Archive\GZipArchive($GLOBALS['BACKUP_DIR'].'/'.$filename);
