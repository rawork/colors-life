...
class Container {
	...
	function callMethodInstance($methodData, $paramsData) {
		...
		if ($methodData['module_name'] != 'tree') {
			if (file_exists($LIB_DIR.'/Controller/'.ucfirst($methodData['module_name']).'.php')) {
				$className = '\\Controller\\'.ucfirst($methodData['module_name']).'Controller';
				$controller = new $className();
			} else{
				$controller = new \Controller\Controller($methodData['module_name']);
			}
	...
	public function get($name) {
		...
		if (!isset($this->services[$name])) {
			throw new \Exception('Не подключена служба "'.$name.'"');
		}
