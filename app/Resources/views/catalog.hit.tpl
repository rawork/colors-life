{if $methodName == 'stuff'}
	{raIds var=ids query="SELECT s.id FROM catalog_category t1 JOIN catalog_product s ON t1.id=s.category_id WHERE s.publish=1 AND s.is_hit=1 AND t1.root_id=`$item.category_id_root_id` AND s.id<>`$item.id`" }
