...
{if $param1 != 'ord' && $param1 != 'price' && $param1 != 'name'}
            ...
            {raPaginator var=pages table=catalog_stuff query="publish='on' AND c_id=`$param0` `$param2_where`" pref="`$ref``$mname`.`$param0`.`$param1`.`$param2`.htm?page=###&rtt=`$rtt`" per_page=$rtt page=$page tpl=public}
            ...
            <td width="60%" align="right">Показать товары: <a href="{raURL node=$node.name method=index prms=$param0}">таблицей</a> <span>списком</span>
