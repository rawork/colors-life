...
class ImageFieldType extends FileFieldType {
	public function __construct(&$params, $entity = null) {
		...
		if (!empty($this->params['sizes'])) {
			$this->params['sizes'] = 'default|100x50,'.$this->params['sizes'];
		}	
	}
	...
	global $PRJ_REF;
		...
		return $photo ? '<a target="_blank" href="'.$this->dbValue.'"><img '.$width.' border="0" src="'.$PRJ_REF.$photo.'"></a><span class="imageinfo">('.$this->get('util')->getFileSize($this->dbValue).')</span>'.$extra_text : '';
	}
