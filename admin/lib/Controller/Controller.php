...
class Controller {
	...
	function __construct($name) {
		...
		$this->get('smarty')->assign('controller', $this->get('router')->getParam('controller'));
		...
		if ($this->name == 'catalog' && $this->get('router')->getParam('methodName') == 'index') {
			if (isset($params[0])) {
				...
				foreach ($path as $k => $item) {
					...
					$path[$k]['ref'] = $this->get('container')->href($this->get('router')->getParam('nodeName'), 'index', array($item['id']));
				}
				$nodes = $path;
			}
