    ...
    class mysqliConnector extends DBConnector{
        ...
        public function execQuery($name, $query) {
            ...
            if ($this->connection) {
				...
				if (stristr($query, ';#|#|#')) {
					...
					$this->result[$name] = $this->connection->multi_query($query);
				} else {
					$this->result[$name] = $this->connection->query($query);
				}
		...
		public function backupDB($filename) {
			...
			foreach ($tablelist as $tableitem) {
				...
				fwrite($f, 'CREATE TABLE `'.$tableitem[0].'` ('.$fields_text.') ENGINE=MyISAM DEFAULT CHARSET=cp1251;'."\n\n");
