<?php
	class DBRTTI {
				...
				if (inc_lib('db/DBStructure/'.$u['name'].'.php')) {
					...
					if (isset($$tables_array_name)) {
						...
						foreach ($tables as $k => $table) {
							...
							$ret[$table['component'].'_'.$table['name']] = new DBTable($table);
						}
					}
				}
            }
		...
		function getTable($tname) {
			return $this->dbtables[$tname];
		}
		
		function getTables($cname) {
			$tables = array();
			foreach ($this->dbtables as $k => $dbtable) {
		...
		function getItem($class, $condition = 0, $sort = '', $select = '') {
			$ret = array();
			if (!isset($this->dbtables[$class])) {
				CUtils::raiseError('Class not found: '.$class, ERROR_RETURN);
			} else {
				$ret = $this->dbtables[$class]->getItem($condition, $sort, $select);
			}
			return $ret;
		...
		function getItems($class, $condition = '', $sort = '', $limit = false, $select = '', $detail = true) {
			...
			if (!isset($this->dbtables[$class])) {
				...
				if (trim($select) != '') 
					$a['select'] = $select;
				$this->dbtables[$class]->select($a);
				$ret = $this->dbtables[$class]->getNextArrays($detail);
			}
			return $ret;
		}
		...
		function getCount($class, $condition = '') {
			...
			if (!isset($this->dbtables[$class])) {
				...
				$ret = $this->dbtables[$class]->getCount($condition);
			}
			return $ret;
		}
		...
		function addItem($class, $fields, $values) {
			if (!isset($this->dbtables[$class])) {
				...
				return $this->dbtables[$class]->insert($fields, $values);
			}
		}
		...
		function addGlobalItem($class) {
			if (!isset($this->dbtables[$class])) {
				...
				return $this->dbtables[$class]->insertGlobals();
			}
		}
		...
		function updateItem($class, $query = 0, $condition) {
			if (!isset($this->dbtables[$class])) {
				...
				if (is_numeric($query)) {
					return $this->dbtables[$class]->update($condition.' WHERE id='.$query);
				} else {
					return $this->dbtables[$class]->update($condition.' WHERE '.$query);
				}
			}
		}
		...
		global $db;
			if (!isset($this->dbtables[$class])) {
				...
				if ($ids = $this->delRel($class, $this->getItems($class, !empty($q) ? $q : '1<>1'))) 
					return $this->dbtables[$class]->delete('id IN ('.$ids.')');
				else
					return false;
			}
		...
		function delRel($class, $items = array()) {
			$ids0 = '';
			foreach ($items as $a) {
                if ($this->dbtables[$class]->props['is_system']) {
                ...
                foreach ($this->dbtables[$class]->fields as $f) {
                    $ft = $this->dbtables[$class]->createFieldType($f, $a);
		...
		global $db;	
			...
			if (sizeof($a) > 0) {
				for ($i = 1; $i <= $times; $i++)
					$this->dbtables[$class]->insertArray($a);
		...
		function updateClass($class) {
			if (!isset($this->dbtables[$class])) {
				CUtils::raiseError('Can not update class: '.$class, ERROR_RETURN);
			} else {
				$this->dbtables[$class]->alterNew();
			}
		}
		...
		function deleteClass($class, $simple = true) {
			if (!$simple) {
				...
				if (sizeof($a) == 0){ 
					CUtils::raiseError('Class not found: '.$class, ERROR_DIE);
				}
				global $db;
			...
			return $this->dbtables[$class]->drop();
		}

		function truncateClass($class) {
			return $this->dbtables[$class]->truncate();
		}
