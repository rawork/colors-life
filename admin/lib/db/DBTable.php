<?php
    ...
    inc_lib('db/DBField/checkbox.php');
    inc_lib('db/DBField/color.php');
	inc_lib('db/DBField/currency.php');
	inc_lib('db/DBField/date.php');
	inc_lib('db/DBField/datetime.php');
    inc_lib('db/DBField/enum.php');
    inc_lib('db/DBField/file.php');
	inc_lib('db/DBField/gallery.php');
	inc_lib('db/DBField/html.php');
	inc_lib('db/DBField/image.php');
	inc_lib('db/DBField/number.php');
    inc_lib('db/DBField/password.php');
    inc_lib('db/DBField/select.php');
	inc_lib('db/DBField/select_list.php');
    inc_lib('db/DBField/select_tree.php');
    inc_lib('db/DBField/string.php');
    inc_lib('db/DBField/template.php');
    inc_lib('db/DBField/text.php');
    class DBTable {
        public $name;
		public $cname;
		public $Id;
        ...
        function __construct($table) {
			...
			$this->_sTableName		= $table['name'];
			$this->_sModuleName		= $table['component'];
			$this->setDBTableName($this->_sModuleName, $this->_sTableName);
...
//			return $this->_aFieldTypes[$aProperties['type']];
		...
		global $db;
			...
			CLog::write("Изменение записи $iEntityId таблицы ".$this->getDBTableName());
        ...
        function drop() {
		global $db;
			CLog::write('Удаление таблицы '.$this->getDBTableName());
			return $db->execQuery($this->getDBTableName().'_droptable', 'DROP TABLE '.$this->getDBTableName());
        }
        function truncate() {
			global $db;
			CLog::write('Очистка таблицы '.$this->getDBTableName());
		...
		global $db;
			...
			";
			//die($sQuery);
			CLog::write("Обновление записей с условием \"$sQuery\" в таблице ".$this->getDBTableName());
			return $db->execQuery($sDBTableName, $sQuery);
        }
        function delete($sQuery) {
		global $db;
			CLog::write("Удаление записей с условием \"$sQuery\" из таблицы ".$this->getDBTableName());
		...
		public function setDBTableName($sModuleName, $sTableName) {
			$this->_sDBTableName = $sModuleName.'_'.$sTableName;
		}

		public function getDBTableName() {
			return $this->_sDBTableName;
		}
