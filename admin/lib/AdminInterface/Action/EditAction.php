...
class EditAction extends Action {
	...
	global $PRJ_DIR;
		...
		if (count($a)) {
			if (file_exists($PRJ_DIR.'/app/Resources/views/admin/components/'.$this->get('router')->getParam('module').'.'.$this->get('router')->getParam('table').'.tpl')){
				...
				return $ret.$this->get('smarty')->fetch('admin/components/'.$this->get('router')->getParam('module').'.'.$this->get('router')->getParam('table').'.tpl');
			} else {
	...
	function getSizesForm() {
		...
		$sizes = $this->get('connection')->getItems('sizelist', $sql);
	...
	$ret .= '</tr>';
			...
			$sql = "SELECT * FROM system_files WHERE table_name='".$this->dataTable->getDBTableName()."' AND record_id=".$a['id']." ORDER BY credate";
			...
			foreach ($my_files as $fileitem) {
				...
				$ret .= '<td><a href="#" onClick="delFile(\''.$fileitem['id'].'\',\''.$fileitem['name'].'\',\''.$this->dataTable->getDBTableName().'\',\''.$a['id'].'\'); return false"><img src="'.$THEME_REF.'/img/icons/icon_delete.gif" border="0"></a></td>'."\n";
				$ret .= '</tr>';	
			}
			$ret .= '</table>';
			$ret .= '</div>';
			$ret .= '<input type="button" id="updatelistbtn" class="adm-btn" onclick="updateFileList(\''.$this->dataTable->getDBTableName().'\','.$a['id'].');return false" value="Обновить список" />'."\n";
			$ret .= '<br><br><fieldset><legend>Добавить файл</legend>';
			$ret .= '<form id="uploadForm" action="/admin/doajaxfileupload.php" method="post" enctype="multipart/form-data">'."\n";
			$ret .= '<input name="table_name" value="'.$this->dataTable->getDBTableName().'" type="hidden"/>'."\n";
	...
	function getText() {
		...
		if (file_exists($PRJ_DIR.'/app/Resources/views/admin/'.$this->get('router')->getParam('module').'.'.$this->get('router')->getParam('table').'.edit.tpl')){
			$this->get('smarty')->assign('update_form', $this->getUpdateForm());
			$this->get('smarty')->assign('sizes_form', $this->getSizesForm());
			$this->get('smarty')->assign('files_form', $this->getFilesForm());
			$ret .= $this->get('smarty')->fetch('admin/'.$this->get('router')->getParam('module').'.'.$this->get('router')->getParam('table').'.edit.tpl');
		} else {
