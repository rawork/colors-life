...
class AdminAjaxController extends Controller {
	...
	function getComponentList($state, $moduleName = '') 
	{
		...
		$this->get('smarty')->assign('state', $state);
		$this->get('smarty')->assign('module', $moduleName);
		$ret = array();
		switch ($state) {
		...
		$text = $this->get('smarty')->fetch('admin/mainmenu.tpl');
	...
	function getTableList($state, $moduleName) {
		if ($this->get('util')->_sessionVar('user')) {
			$this->get('router')->setParam('state', $state);
			$this->get('router')->setParam('module', $moduleName);
			$uai = new \AdminInterface\AdminController(new \Admin\Admin($moduleName), '', array());
	...
	function showTreePopup($input_id, $table_name, $field_name, $dbid, $zero_title, $value) {
		$t = $this->get('container')->getTable($table_name);
	...
	// Множественный выбор
	function showListPopup($inputId, $tableName, $fieldName, $dbId, $value) {
		...
		$text .= $this->list_popup_show($field, $values);
		$text .= '</table>';
		
		return json_encode( array(
			'title' => $field['title'], 
			'button' => '<input type="button" class="adm-btn" value="Выбрать" onclick="make_list_choice('."'".$inputId."'".')">',
			'content' => $text
		));
	}
    ...
    function tree_popup_show_subtree($p_id, $prefix_width, $exclude, $f, $value) {
		$ret = '';
		if (!empty($f['l_lang'])) {
			$lang_where = " AND lang='".$this->get('util')->_sessionVar('lang', false, 'ru')."'";
		} else {
			$lang_where = '';
		}
		$f['l_sort'] = !empty($f['l_sort']) ? $f['l_sort'] : $f['l_field'];
		$trees = $this->get('container')->getItems($f['l_table'], 'p_id='.$p_id.$lang_where, $f['l_sort']);
    ...
    function list_popup_show($f, $values) {
		$ret = '';
		$lang_where = !empty($f['l_lang']) ? "lang='".$this->get('util')->_sessionVar('lang', false, 'ru')."'" : '';
		if (!empty($f['query'])) {
			$lang_where .= ($lang_where ? ' AND ' : '').'('.$f['query'].')';
		}
		$f['l_sort'] = !empty($f['l_sort']) ? $f['l_sort'] : $f['l_field'];
        $items = $this->get('container')->getItems($f["l_table"], $lang_where, $f["l_sort"]);
		$fields = explode(",", $f["l_field"]);
        foreach ($items as $a) {
			$vname = '';
			foreach ($fields as $fi) {
	...
	function makeArchive() {
		...
		fclose($f);
		set_time_limit(0);
		$this->get('connection')->backupDB($filename_sql);
		$archive = new \Common\Archive\GZipArchive($filename);
		...
		$text .= '<br>';
		$text .= 'Размер архива: '.$this->get('util')->getFileSize('/'.$filename, 2);
